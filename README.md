...
To use the core constructs in a new project, simply migrate all of the content of the template project to the new project (the test recordings can be omitted if desired or replaced later). Then in the new project, under project settings, set the Defaualt GameMode to BP_FirstPersonGameMode and the Default Pawn Class to BP_FirstPersonCharacter_30ASpheres and make the Master Submix Custom_Master_Submix. Finally, attach the EventGraph of the Example_Sound_Emitter blueprint to any sound source you wish to encode into 3rd-order ambisonics and change the audio component node to match your sound source.
___
